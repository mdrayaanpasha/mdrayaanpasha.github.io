...
const OPACITY_EASE = [0.4, 0, 0.2, 1];
...
export default function MouseEffectCard({
  ...
  return (
    <Card
      className={cn(
        "group relative w-full max-w-md overflow-hidden rounded-2xl border border-white/40 p-0 shadow-none dark:border-white/10",
        className
      )}
    >
      <CardContent
        ...
        {image && (
          <div className="absolute inset-0 z-0">
            <img
              ...
              alt={imageAlt}
              loading="lazy"
              className="h-full w-full object-cover opacity-45 grayscale transition-all duration-700 group-hover:opacity-70 group-hover:grayscale-0"
            />
            {/* Legibility overlay so text/dots read on top of the image */}
            <div className="absolute inset-0 bg-white/50 dark:bg-zinc-950/65" />
          </div>
        )}
